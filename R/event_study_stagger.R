...
#'
#' @examples
#' event_study_stagger(daily_crime,  window = 8, cluster_blocks = 7, reference_days =1 )
#'
...
event_study_stagger_day<- function(data, window, cluster_blocks, reference_day) {
  ...
  event_study <- data %>%
    ...
    mutate(treatment = ifelse(date >= closure_1, 1, 0)) %>%
  ...
  for (i in window) {
    ...
    event_study <- event_study %>%
      mutate(!!sym(closure_plus) := (closure_1 + (i) *days(cluster_blocks))) %>%
      mutate(!!sym(closure_plus_end) := closure_1 + ((i+1) * days(cluster_blocks))) %>%
  ...
  event_study <- event_study %>%
    mutate(treatment = ifelse(date >= closure_1 & date < (closure_1 + days(7)), 1, 0))
